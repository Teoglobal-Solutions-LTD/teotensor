    ...
    *,
    ...
    if metrics or diagnostics:
        ...
        if metrics:
            ...
            body_parts.append(_panel_heading("Metrics"))
            body_parts.append("<div class='metrics'>")
            for key, value in sorted(metrics.items()):
                body_parts.append(_metric_card(key, value, extras))
            body_parts.append("</div></section>")
        if diagnostics:
            body_parts.append("<section class='panel'>")
            body_parts.append(_panel_heading("Diagnostics"))
    ...
    if figures:
        body_parts.append("<section class='panel'>")
        body_parts.append(_panel_heading("Figures"))
        n_fig = len(figures)
    ...
    if traces or tables:
        ...
        if traces:
            body_parts.append("<section class='panel'>")
            body_parts.append(_panel_heading("Traces"))
            for trace in traces:
                body_parts.append(_trace_block(trace))
            body_parts.append("</section>")
        if tables:
            body_parts.append("<section class='panel'>")
            body_parts.append(_panel_heading("Tables"))
            for table in tables:
                body_parts.append(
    ...
    body_parts.append(
        ...
        "Use Print / Save as PDF for a one-page archive. "
        "Hover ? icons for field definitions.</footer>"
    )
...
_SECTION_LEDE: dict[str, str] = {
    "Metrics": "Scalar snapshot of the fitted model. Hover ? for definitions.",
    "Diagnostics": "Automated health checks. Warnings deserve a second look.",
    "Figures": "Charts from model artifacts — the estimator never draws them itself.",
    "Traces": "Step-by-step records from iterative algorithms.",
    "Tables": "Tabular detail behind the headline metrics.",
}

_METRIC_HELP: dict[str, str] = {
    "n_components": "How many principal axes were kept after truncation.",
    "n_features": "Number of input features seen during fit.",
    "n_samples": "Number of training rows seen during fit.",
    "variance_retained": (
        "Fraction of total training variance captured by the kept components. "
        "Closer to 1 means less information was discarded."
    ),
    "reconstruction_mse": (
        "Mean squared error after projecting to the kept subspace and back. "
        "Lower is better; 0 means perfect reconstruction."
    ),
    "noise_variance": (
        "Average variance left in the discarded components — a proxy for "
        "residual / noise energy."
    ),
    "final_loss": "Last recorded objective value from an iterative fit.",
    "n_noise": "Number of points labeled as noise (e.g. DBSCAN label -1).",
}
...
_ACRONYMS = frozenset({"mse", "pca", "svd", "evr", "id", "dbscan"})


def _humanize_key(key: str) -> str:
    """Turn ``reconstruction_mse`` into ``Reconstruction MSE``."""
    parts: list[str] = []
...
def _format_scalar(value: Any, *, key: str = "") -> tuple[str, str]:
    """Return ``(display, full)`` strings for a metric / table cell.

    Long floats are compacted for layout; ``full`` keeps exact text for tooltips.
    """
    full = str(value)
    ...
    if isinstance(value, float):
        key_l = key.lower()
        if key_l in _PERCENT_KEYS or key_l.endswith("_ratio"):
            display = f"{value:.1%}"
        elif abs(value) != 0.0 and (abs(value) < 1e-3 or abs(value) >= 1e4):
            display = f"{value:.3e}"
        else:
            display = f"{value:.4g}"
        return display, full
    if hasattr(value, "item"):
        try:
            return _format_scalar(value.item(), key=key)
        except (TypeError, ValueError):
            return full, full
    return full, full


def _metric_help_text(key: str, extras: dict[str, Any]) -> str:
    custom = extras.get("metric_help")
    if isinstance(custom, dict) and key in custom:
        return str(custom[key])
    return _METRIC_HELP.get(
        key,
        "Metric recorded by the model. See the docs for this estimator.",
    )


def _help_chip(text: str) -> str:
    tip = html.escape(text, quote=True)
    return (
        f"<button type='button' class='help-tip no-print' data-tip='{tip}' "
        f"aria-label='Help'>?</button>"
    )


def _panel_heading(title: str, *, lede: str | None = None) -> str:
    text = lede if lede is not None else _SECTION_LEDE.get(title, "")
    parts = [f"<div class='panel-head'><h2>{html.escape(title)}</h2>"]
    if text:
        parts.append(_help_chip(text))
    parts.append("</div>")
    if text:
        parts.append(f"<p class='panel-lede'>{html.escape(text)}</p>")
    return "".join(parts)


def _metric_card(key: str, value: Any, extras: dict[str, Any]) -> str:
    label = _humanize_key(key)
    display, full = _format_scalar(value, key=key)
    help_text = _metric_help_text(key, extras)
    tip_bits = [help_text]
    if display != full:
        tip_bits.append(f"Exact value: {full}")
    tip = html.escape(" · ".join(tip_bits), quote=True)
    return (
        f"<div class='metric' data-key='{html.escape(key)}'>"
        f"<div class='metric-head'>"
        f"<span class='metric-key'>{html.escape(label)}</span>"
        f"{_help_chip(help_text)}"
        f"</div>"
        f"<span class='metric-val' title='{html.escape(full)}' "
        f"data-tip='{tip}'>{html.escape(display)}</span>"
        f"</div>"
    )
...
def _trace_block(trace: Trace) -> str:
    ...
    rows = "".join(
        "<tr>"
        + "".join(
            f"<td title='{html.escape(_format_scalar(v)[1])}'>"
            f"{html.escape(_format_scalar(v)[0])}</td>"
...
def _table_html(table: TableSpec) -> str:
    head = "".join(
        f"<th>{html.escape(_humanize_key(col))}</th>" for col in table.columns
    )
    body_rows: list[str] = []
    for row in table.rows:
        cells: list[str] = []
        for col, cell in zip(table.columns, row, strict=True):
            display, full = _format_scalar(cell, key=str(col))
            cells.append(f"<td title='{html.escape(full)}'>{html.escape(display)}</td>")
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .help-tip {{
      ...
      background: #fff;
      color: var(--teal-deep);
      font: 700 0.68rem/1 "Space Grotesk", "Segoe UI", sans-serif;
      cursor: help;
    }}
    .help-tip:hover,
    ...
    @media print {{
      ...
      .panel-lede {{ color: #334155 !important; }}
